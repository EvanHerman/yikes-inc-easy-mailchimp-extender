...
class Yikes_Inc_Easy_Mailchimp_Forms_Admin {
	...
	/* End Two Week Notificaition */
		...
		public function display_php_warning() {
			$message = __( 'YIKES Inc. Easy MailChimp Forms requires a minimum of PHP 5.3. The plugin will not function properly until you update. Reach out to your host provider for assistance.' , 'yikes-inc-easy-mailchimp-extender' );
		...
		*/
		public function tinymce_yikes_easy_mc() {					
			...
			<script type="text/javascript">
				...
				var localized_data = {
					'button_title' : '<?php _e( 'Easy MailChimp Forms by Yikes Inc.', 'yikes-inc-easy-mailchimp-extender' ); ?>',
					'popup_title' : '<?php _e( 'Easy MailChimp Forms by Yikes Inc.', 'yikes-inc-easy-mailchimp-extender' ); ?>',
	...
	public function register_admin_pages() {	
		...
		/*************/
			...
			/** Hidden Pages **/
				...
				add_submenu_page( 
					'options-writing.php', 
					__( 'YIKES Inc. Easy MailChimp Forms Upgrade Options Structure' , 'yikes-inc-easy-mailchimp-extender' ), 
					'YIKES Inc. Easy MailChimp Forms Upgrade Options Structure', 
					'manage_options', 
	...
	**/
	function yikes_easy_mc_settings_init() { 	
		...
		/* reCAPTCHA Settings */
			...
			register_setting( 'yikes_inc_easy_mc_recaptcha_settings_page' , 'yikes-mailchimp-recaptcha-status' );
			register_setting( 'yikes_inc_easy_mc_recaptcha_settings_page' , 'yikes-mailchimp-recaptcha-site-key' );
			register_setting( 'yikes_inc_easy_mc_recaptcha_settings_page' , 'yikes-mailchimp-recaptcha-secret-key' );
			...
			add_settings_field( 
				'yikes-mailchimp-recaptcha-site-key', 
			...
			add_settings_field( 
				'yikes-mailchimp-recaptcha-secret-key', 
			...
			add_settings_field( 
				'yikes-mailchimp-recaptcha-status', 
	...
	**/
	...
	/**
	* Generate YIKES Inc. Easy MailChimp Forms Options Page
	* 
	* @since    1.0.0
	*/
			...
			if( isset( $_GET['yikes-mc-options-migration-dismissed'] ) && $_GET['yikes-mc-options-migration-dismissed'] == 'true' ) {
				...
				?>
					<div class="yikes-easy-mc-updated migrate-options-notice">
						<p><?php printf( __( "The previously stored options for %s have been cleared from the database. You should update the options on the <a href='%s' title='Settings Page'>settings page</a> before continuing to use the plugin.", 'yikes-inc-easy-mailchimp-extender' ), '<strong>YIKES Inc. Easy MailChimp Forms</strong>', admin_url( 'admin.php?page=yikes-inc-easy-mailchimp-settings' ) ); ?></p>
					</div>
				<?php
			} else {
			?>
				<div class="yikes-easy-mc-updated migrate-options-notice">
					<p><?php printf( __( "It looks like you're upgrading from a previous version of %s.", 'yikes-inc-easy-mailchimp-extender' ), '<strong>YIKES Inc. Easy MailChimp Forms</strong>' ); ?> <?php printf( __( "In the newest version of %s, the options data structure has changed. We've also moved the mailing lists into its own database table to allow for some higher level customization. Now you can easily create multiple forms and assign them to the same mailing list." , 'yikes-inc-easy-mailchimp-extender' ), '<strong>YIKES Inc. Easy MailChimp Forms</strong>' ); ?></p>
			...
			?>
				...
				<div class="inside">
					<ul id="settings-nav">
						...
						<li><?php if( isset( $_REQUEST['section'] ) && $_REQUEST['section'] == 'checkbox-settings' ) { ?><div class="option-menu-selected-arrow"></div><?php } ?><a href="<?php echo esc_url_raw( add_query_arg( array( 'section' => 'checkbox-settings' ) , admin_url( 'admin.php?page=yikes-inc-easy-mailchimp-settings&section=checkbox-settings' ) ) ); ?>"><?php _e( 'Checkbox Settings' , 'yikes-inc-easy-mailchimp-extender' ); ?></a></li>
		...
		public function generate_show_some_love_container() {
			...
			if( get_option( 'yikes-easy-mc-active-addons' , array() ) == array() ) {
				...
				<div class="postbox yikes-easy-mc-postbox">
					...
					<!-- review us container -->
					<h3><?php _e( 'View Easy MailChimp Forms Add-Ons' , 'yikes-inc-easy-mailchimp-extender' ); ?></h3>
					<div id="review-yikes-easy-mc" class="inside">
		...
		*/
		public function build_available_merge_vars( $form_fields , $available_merge_variables ) {
			...
			if( !empty( $available_merge_variables['data'][0] ) ) {	
				...
				?></ul>
				<a href="#" class="add-field-to-editor button-secondary yikes-easy-mc-hidden"><small><?php _e( 'Add to Form Builder' , 'yikes-inc-easy-mailchimp-extender' ); ?></small></a>
				<?php
			}
		}
		...
		*/
		public function build_available_interest_groups( $form_fields , $available_interest_groups , $list_id ) {
			...
			if( !empty( $available_interest_groups) ) {	
				...
				?></ul>
				<a href="#" class="add-interest-group-to-editor button-secondary yikes-easy-mc-hidden"><small><?php _e( 'Add to Form Builder' , 'yikes-inc-easy-mailchimp-extender' ); ?></small></a>
				<?php
			}
		}
