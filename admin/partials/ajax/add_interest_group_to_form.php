...
$interest_groupings = $list_helper->get_interest_categories( $form_data['list_id'] );
if ( is_wp_error( $interest_groupings ) ) {
	...
	<section class="draggable" id="error-container">
		<p>
			<span class="dashicons dashicons-no-alt"></span> <?php printf( __( 'Error: %s', 'yikes-inc-easy-mailchimp-extender' ), $interest_groupings->get_error_code() ); ?>
		</p>
	</section>
	<?php
	return;
...
$index      = $this->findMCListIndex( $form_data['group_id'], $interest_groupings, 'id' );
$field_data = $interest_groupings[ $index ];
$groups     = wp_list_pluck( $field_data['items'], 'name' );


?>
<section class="draggable" id="<?php echo $form_data['group_id']; ?>">
	<!-- top -->
	...
	<div class="yikes-mc-settings-expansion-section">
		...
		<p class="type-container"><!-- necessary to prevent skipping on slideToggle(); -->
			<!-- store the label -->
			<input type="hidden" name="field[<?php echo $form_data['group_id']; ?>][label]" value="<?php echo $form_data['field_name']; ?>" />
			<input type="hidden" name="field[<?php echo $form_data['group_id']; ?>][type]" value="<?php echo $form_data['field_type']; ?>" />
			<input type="hidden" name="field[<?php echo $form_data['group_id']; ?>][group_id]" value="<?php echo $form_data['group_id']; ?>" />
			<input type="hidden" name="field[<?php echo $form_data['group_id']; ?>][groups]" value='<?php echo str_replace( '\'' , '~' , json_encode( $groups ) ); ?>' />
			...
			<table class="form-table form-field-container">
				...
				<?php switch( $form_data['field_type'] ) {
						...
						<tr valign="top">
							...
							<td>
								...
								foreach ( $field_data['items'] as $id => $interest_group ) {
									...
									?>
									<input type="radio" name="field[<?php echo $form_data['group_id']; ?>][default_choice][]" value="<?php echo esc_attr( $id ); ?>" <?php checked( $pre_selected, $id ); ?>><?php echo stripslashes( $interest_group['name'] ); ?>
									<?php
						...
						<tr valign="top">
							...
							<td>
								...
								foreach ( $field_data['items'] as $id => $interest_group ) {
									$pre_selected = ! empty( $field_data['default_choice'] ) ? $field_data['default_choice'] : '0';
									?>
									<input type="checkbox" name="field[<?php echo $form_data['group_id']; ?>][default_choice][]" value="<?php echo $id; ?>" <?php checked( $pre_selected, $id ); ?>><?php echo stripslashes( $interest_group['name'] ); ?>
									<?php
								}
								?>
								<p class="description"><small><?php _e( "Select the option that should be selected by default.", 'yikes-inc-easy-mailchimp-extender' );?></small></p>
					...
					case 'dropdown':
						...
						<tr valign="top">
							...
							<td>
								<select type="default" name="field[<?php echo $form_data['group_id']; ?>][default_choice]">
									<?php
									foreach ( $field_data['items'] as $id => $interest_group ) {
										$pre_selected = ! empty( $field_data['default_choice'] ) ? $field_data['default_choice'] : '0';
										?>
				...
				<tr valign="top">
					...
					<td>
						<textarea class="widefat field-description-input" name="field[<?php echo $form_data['group_id']; ?>][description]"></textarea>
				...
				<tr valign="top">
					...
					<td>
						<input type="text" class="widefat" name="field[<?php echo $form_data['group_id']; ?>][additional-classes]" value="<?php echo isset( $form_data['classes'] ) ? stripslashes( wp_strip_all_tags( $form_data['classes'] ) ) : '' ; ?>" />
					...
					<tr valign="top">
						...
						<td>
							<input type="checkbox" class="widefat" value="1" name="field[<?php echo $form_data['group_id']; ?>][require]">
					...
					<tr valign="top">
						...
						<td>
							<input type="checkbox" class="widefat" value="1" name="field[<?php echo $form_data['group_id']; ?>][hide]">
					...
					<tr valign="top">
						...
						<td>
							<input type="checkbox" name="field[<?php echo $form_data['group_id']; ?>][hide-label]" value="1" />
					...
					<tr valign="top">
						...
						<td>
							<span class="toggle-container">
								...
								<a href="#" class="remove-field" alt="<?php echo $form_data['group_id']; ?>"><?php _e( "Remove Field" , 'yikes-inc-easy-mailchimp-extender' ); ?></a>
							</span>
						</td>
					</tr>
			</table>
